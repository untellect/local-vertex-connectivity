...
namespace vertex_connectivity {
	...
	adjacency_list get_subgraph(const adjacency_list& G, const vertex_vec& V) {
		size_t n = G.size(); // old n
		size_t n_v = V.size(); // new n;

		vertex_vec old_to_new_mapping(n, SIZE_MAX);
	...
	adjacency_list largest_connected_component_undirected(const adjacency_list& G) {
		...
		vertex_vec& LCC_vertices = connected_components[i_LCC];
