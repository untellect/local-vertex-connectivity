...
def normalise(data):
	...
	if len(data) == 0:
		return
	algs = {alg for meta, sample in data for alg in sample}
	#algs = { sample.keys() for meta, sample in data }
	avg_values = { alg:average(sample[alg]["seconds"] for meta, sample in data) for alg in algs}
	for meta, sample in data:
		for alg in algs:
			if (alg in sample) and ("seconds" in sample[alg]) and (avg_values[alg] != 0):
				sample[alg]["seconds"] /= avg_values[alg]
	return
...
if len(argv) == 2:
	...
	data = [(meta, sample) for meta, sample in data if meta['kappa'] != None] # Prune trivial rows.
	normalise(data)
	...
	with open(path + "/norm-results.csv", mode='w') as results:
